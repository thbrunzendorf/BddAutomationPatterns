...
When("the client specifies {} at {} as delivery time", (date: string, time: string) => {
  ...
  cy.get("#DeliveryTime").clear().type(`${deliveryDate.getHours()}:${deliveryDate.getMinutes()}`);
  cy.get("#SaveButton").click();
});

Then("the order should indicate that the delivery date is {}", (date: string) => {
